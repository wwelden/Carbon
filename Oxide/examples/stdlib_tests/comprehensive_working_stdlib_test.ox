...
// Using correct Carbon syntax with fn() -> for lambdas
...
let mappedNumbers = map(fn(x) -> x * 2)(numbers)
print("map(fn(x) -> x * 2): " + toString(mappedNumbers))

let filteredNumbers = filter(fn(x) -> x > 3)(numbers)
print("filter(fn(x) -> x > 3): " + toString(filteredNumbers))

let sumResult = fold(fn(acc, x) -> acc + x)(0)(numbers)
print("fold(fn(acc, x) -> acc + x)(0): " + toString(sumResult))
...
let compositeOp = map(fn(x) -> x * 2)(filter(fn(x) -> x > 2)(numbers))
print("Composite operation - map * filter: " + toString(compositeOp))
